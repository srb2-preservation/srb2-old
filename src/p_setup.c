...
static void P_ClearSingleMapHeaderInfo(short i)
{
	...
	mapheaderinfo[i-1].levelselect = false;
	mapheaderinfo[i-1].noreload = false;
	mapheaderinfo[i-1].timeattack = false;
	mapheaderinfo[i-1].levelselect = false;
	mapheaderinfo[i-1].runsoc[0] = '#';
}
...
static void P_LoadThings(int lump)
{
	...
	for (i = 0; i < nummapthings; i++, mt++)
	{
		...
		{
			case 72: // MT_AXIS
			case 61: // MT_AXISTRANSFER
			case 46: // MT_AXISTRANSFERLINE
				mt->mobj = NULL;
				P_SpawnMapThing(mt);
				break;
			default:
	...
	for (i = 0; i < nummapthings; i++, mt++)
	{
		...
		if (mt->type == 72 // MT_AXIS
			|| mt->type == 61 // MT_AXISTRANSFER
			|| mt->type == 46) // MT_AXISTRANSFERLINE
...
static boolean P_LoadBlockMap(int lump)
{
	...
	if ((lump&0xffff) >= wadfiles[lump>>16]->numlumps
		|| strcmp(W_CheckNameForNum(lump), "BLOCKMAP") != 0)
	{
		return false;
	}
...
static void P_LevelInitStuff(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].timeover = false;

		if (gametype == GT_RACE && players[i].lives < 3)
			players[i].lives = 3;

		players[i].exiting = 0;
		if (!modifiedgame && players[i].dbginfo && (gamemap == 0x01 || gamemap == bkdata[017332]-044))
			players[i].dbginfo = 0;
