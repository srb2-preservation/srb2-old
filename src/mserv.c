...
INT32 GetRoomsList(boolean hosting)
{
	...
	{
		//CONS_Alert(CONS_ERROR, M_GetText("Cannot connect to the Master Server\n"));
		CONS_Printf(M_GetText("Cannot connect to the Master Server\n"));
	...
	for (i = 0; i < NUM_LIST_ROOMS && MS_Read(&msg) >= 0; i++)
	{
		...
		{
			char banmsg[1000];
			memcpy(&banned_info[0], msg.buffer, sizeof (msg_ban_t));
			if (hosting)
		...
		memcpy(&room_list[i], msg.buffer, sizeof (msg_rooms_t));
...
const char *GetMODVersion(void)
{
	...
	if (MS_Write(&msg) < 0)
	{
		...
		CONS_Printf(M_GetText("Could not send to the Master Server\n"));
	...
	if (MS_Read(&msg) < 0)
	{
		...
		CONS_Printf(M_GetText("No reply from the Master Server\n"));
...
static void Command_Listserv_f(void)
{
	if (con_state == MSCS_WAITING)
	{
		...
		CONS_Printf(M_GetText("Not yet connected to the Master Server.\n"));
		return;
	}
	...
	if (GetServersList())
		...
		CONS_Printf(M_GetText("Cannot get server list\n"));
...
static INT32 ConnectionFailed(void)
{
	...
	CONS_Printf(M_GetText("Connection to Master Server failed\n"));
	CloseConnection();
	return MS_CONNECT_ERROR;
}
#endif
...
static INT32 AddToMasterServer(boolean firstadd)
{
	...
	if (res != ERRSOCKET && !res)
	{
		if (retry++ > 30) // an about 30 second timeout
		{
			...
			CONS_Printf(M_GetText("Master Server timed out\n"));
	...
	if (res == ERRSOCKET)
	{
		if (MS_Connect(GetMasterServerIP(), GetMasterServerPort(), 0))
		{
			...
			CONS_Printf(M_GetText("Master Server socket error #%u: %s\n"), errno, strerror(errno));
			MSLastPing = timestamp;
			return ConnectionFailed();
		}
	}
	...
	memcpy(&info->header.signature, &signature, sizeof (UINT32));

	strcpy(info->ip, "");
	strcpy(info->port, int2str(current_port));
	strcpy(info->name, cv_servername.string);
	memcpy(&info->room, & room, sizeof (INT32));
#if VERSION > 0 || SUBVERSION > 0
	sprintf(info->version, "%f.%d", (double)VERSION/100, SUBVERSION%100);
