...
{
	{IT_CALL | IT_STRING, NULL, "New Game",   M_NewGame,      60},
	{IT_CALL | IT_STRING, NULL, "Time Attack",M_TimeAttack,   70},
	{IT_CALL | IT_STRING, NULL, "Statistics", M_Statistics,   80},
	{IT_CALL | IT_STRING, NULL, "Load Game",  M_LoadGame,    100},
	{IT_CALL | IT_STRING, NULL, "Save Game",  M_SaveGame,    110},
...
static void M_DrawConnectMenu(void)
{
	...
	{
		strncpy(servername, serverlist[i].info.servername, 20);
		servername[20] = '\0';
...
char *warningmsg = "No Clans, advertising websites,\nChat, Hangout, Hacked,\nor Role Playing Servers Allowed.\nThis could lead to\nyou getting banned\nfrom the master server.";
...
static void M_DrawServerMenu(void)
{
	...
	V_DrawCenteredString (BASEVIDWIDTH/2, 4, 0, "No Clans, advertising websites, Chat,");
	V_DrawCenteredString (BASEVIDWIDTH/2, 12, 0, "Hangout, Hacked, or Role Playing");
	V_DrawCenteredString (BASEVIDWIDTH/2, 20, 0, "Servers Allowed. This could lead to");
	V_DrawCenteredString (BASEVIDWIDTH/2, 28, 0, "you getting banned from the master");
	V_DrawCenteredString (BASEVIDWIDTH/2, 36, 0, "server.");
...
static void M_DrawStats(void)
{
	...
	if (totalplaytime/(3600*TICRATE) < 10)
		sprintf(hours, "0%d", (int)(totalplaytime/(3600*TICRATE)));
	else
		sprintf(hours, "%d:", (int)(totalplaytime/(3600*TICRATE)));

	if (totalplaytime/(60*TICRATE)%60 < 10)
		sprintf(minutes, "0%d", (int)(totalplaytime/(60*TICRATE)%60));
	else
		sprintf(minutes, "%d", (int)(totalplaytime/(60*TICRATE)%60));

	if (((totalplaytime/TICRATE) % 60) < 10)
		sprintf(seconds, "0%d", (int)((totalplaytime/TICRATE) % 60));
	else
		sprintf(seconds, "%d", (int)((totalplaytime/TICRATE) % 60));
	...
	if (displaytimeattack)
	{
		if (besttime/(3600*TICRATE) < 10)
			sprintf(hours, "0%d", (int)(besttime/(3600*TICRATE)));
		else
			sprintf(hours, "%d", (int)(besttime/(3600*TICRATE)));

		if (besttime/(60*TICRATE)%60 < 10)
			sprintf(minutes, "0%d", (int)(besttime/(60*TICRATE)%60));
		else
			sprintf(minutes, "%d", (int)(besttime/(60*TICRATE)%60));

		if (((besttime/TICRATE) % 60) < 10)
			sprintf(seconds, "0%d", (int)((besttime/TICRATE) % 60));
		else
			sprintf(seconds, "%d", (int)((besttime/TICRATE) % 60));
		...
		for (i = oldlastmapnum; i < NUMMAPS; i++)
		{
			...
			if (timedata[i].time)
			{
				if (timedata[i].time/(60*TICRATE) < 10)
					sprintf(minutes, "0%d", (int)(timedata[i].time/(60*TICRATE)));
				else
					sprintf(minutes, "%d", (int)(timedata[i].time/(60*TICRATE)));

				if (((timedata[i].time/TICRATE) % 60) < 10)
					sprintf(seconds, "0%d", (int)((timedata[i].time/TICRATE) % 60));
				else
					sprintf(seconds, "%d", (int)((timedata[i].time/TICRATE) % 60));

				if ((timedata[i].time % TICRATE) < 10)
					sprintf(hours, "0%d", (int)(timedata[i].time % TICRATE));
				else
					sprintf(hours, "%d", (int)(timedata[i].time % TICRATE));
...
void M_DrawTimeAttackMenu(void)
{
	...
	if (W_CheckNumForName(description[cv_chooseskin.value-1].picname) != -1)
...
//  A 160x100 image of the level as entry MAPxxP
	...
	sprintf(hours,   "%02d", (int)(besttime/(3600*TICRATE)));
	sprintf(minutes, "%02d", (int)(besttime/(60*TICRATE)%60));
	sprintf(seconds, "%02d", (int)((besttime/TICRATE) % 60));
	sprintf(tics,    "%02d", (int)(besttime%TICRATE));
	...
	if (cv_nextmap.value)
	{
		if (timedata[cv_nextmap.value-1].time > 0)
			V_DrawCenteredString(BASEVIDWIDTH/2, 116, 0, 	va("Best Time: %d:%02d.%02d", (int)(timedata[cv_nextmap.value-1].time/(60*TICRATE)%60),
				(int)((timedata[cv_nextmap.value-1].time/TICRATE) % 60), (int)(timedata[cv_nextmap.value-1].time % TICRATE)));
	}
...
static menuitem_t ScreenshotOptionsMenu[] =
{
	...
	{IT_STRING|IT_CVAR, NULL, "Memory Level",      &cv_zlib_memory,                45},
	{IT_STRING|IT_CVAR, NULL, "Compression Level", &cv_zlib_level,                 55},
	{IT_STRING|IT_CVAR, NULL, "Strategy",          &cv_zlib_strategy,              65},
	{IT_STRING|IT_CVAR, NULL, "Window Size",       &cv_zlib_window_bits,           75},

	{IT_HEADER, NULL, "Movie Mode (F9)", NULL, 85},
	{IT_STRING|IT_CVAR, NULL, "Storage Location",  &cv_movie_option,              95},
	{IT_STRING|IT_CVAR|IT_CV_STRING, NULL, "Custom Folder", &cv_movie_folder, 	  105},
	{IT_STRING|IT_CVAR, NULL, "Capture Mode",      &cv_moviemode,                 130},

	{IT_STRING|IT_CVAR, NULL, "Region Optimizing", &cv_gif_optimize,              140},
	{IT_STRING|IT_CVAR, NULL, "Downscaling",       &cv_gif_downscale,             150},

	{IT_STRING|IT_CVAR, NULL, "Memory Level",      &cv_zlib_memorya,              140},
	{IT_STRING|IT_CVAR, NULL, "Compression Level", &cv_zlib_levela,               150},
	{IT_STRING|IT_CVAR, NULL, "Strategy",          &cv_zlib_strategya,            160},
	{IT_STRING|IT_CVAR, NULL, "Window Size",       &cv_zlib_window_bitsa,         170},
};
...
menu_t EraseDataDef =
{
	"M_OPTTTL",
	"OPTIONS",
	sizeof (EraseDataMenu)/sizeof (menuitem_t),
	&DataOptionsDef,
	EraseDataMenu,
	M_DrawGenericMenu,
	60, 40,
...
#if defined (UNIXLIKE) || defined (SDL) && !defined (__EMSCRIPTEN__)
...
//	{IT_STRING | IT_CVAR,    NULL, "Adv. Precip Calcs", &cv_advancedprecip, 100},
};
...
//
static void M_ToggleSFX(int choice)
{
	(void)choice;
	if (nosound)
	{
...
static void M_ToggleDigital(int choice)
{
	(void)choice;
	if (nofmod)
	{
...
static void M_ToggleMIDI(int choice)
{
	(void)choice;
	if (nomusic)
	{
...
static menuitem_t JoystickMenu[] =
{
	...
	{IT_STRING      | IT_CVAR, NULL, "Axis For Turning", &cv_turnaxis  ,  40},
	...
	{IT_STRING      | IT_CVAR, NULL, "Axis For Turning", &cv_turnaxis2 , 120},
...
static void M_ControlDef2(int choice);
...
//			(void)READBYTE(save_p);
	...
	//P_UnArchivePlayers()
	{
		...
		(void)READBYTE(save_p); // charflags
		(void)READBYTE(save_p); // supercolor
...
#ifdef SDLIO
		count = SDL_RWread(handle, &savegamestrings[i], 1,
			SAVESTRINGSIZE);
		SDL_RWclose(handle);
#else
...
static void M_SaveGame(int choice)
{
	...
	if (timeattacking)
	{
		M_StartMessage("You can't save while time attacking!\nPress a key\n",NULL,MM_NOTHING);
		return;
	}

	if (gameskill == sk_insane)
	{
		M_StartMessage("You can't save while on Ultimate skill!\nPress a key\n", NULL,MM_NOTHING);
		return;
	}
